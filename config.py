...
# ทุกรอบที่บอทสแกน (ทุก LOOP_INTERVAL_SECONDS วินาที) จะไล่ดูเหรียญทั้งกระดาน THB แล้วเลือก
# เหรียญที่ "ติดลบมากที่สุด ณ ขณะนั้นบนกระดาน" (% เปลี่ยนแปลงที่ Bitkub แสดงเองตรงๆ ไม่ใช่คำนวณ
# จากหน้าต่างเวลาตายตัว เพราะเหรียญอาจตกมาก่อนหน้านั้นแล้วก็ได้) มาซื้อก่อน ถ้าซื้อไม่ได้ไล่ตัว
# ถัดไปที่ตกรองลงมา ขายเมื่อราคาพลิกเป็นบวกหลังหักค่าธรรมเนียมแล้วเท่านั้น (ไม่ขาดทุน)
# ค่าทั้งหมดด้านล่างนี้เป็นค่าคงที่ในโค้ด (ไม่อ่านจาก Railway Variables) แก้ไขได้โดยตรงที่นี่แล้ว redeploy
# ---------------------------------------------------------------------
...
DIP_BUY_USE_ALL_AVAILABLE_THB = True
